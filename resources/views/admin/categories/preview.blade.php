...
<div class="card w-100" id="lang_prev">
    <div class="card-header d-flex justify-content-between gap-3">
        ...
        <a href="/admin/categories/add" class="btn btn-primary w-fit d-flex gap-2 align-items-center">
            <i class="ti ti-plus"></i> اضافة قسم
        </a>
    </div>
            ...
            <tr v-for="(category, index) in categories_data" :key="index">
                ...
                <td class="border-bottom-0">
                    <div class="d-flex gap-2">
                        <form :action="`/admin/settings/toggleCatAtMain/${category.id}`" method="post">
                            @csrf
                            <button type="submit" v-if="category.isAtNavMain" class="btn btn-danger " style=" display: block">الغاء الاولوية</button>
                            <button type="submit" v-if="!category.isAtNavMain" class="btn btn-success" style=" display: block">اعطاء الاولوية</button>
                        </form>
    ...
    <h4 class="text-center">
        @{{ !categories_data || categories_data.length == 0 ? 'لا يوجد اي أقسام' : '' }}
    </h4>
        ...
        <div class="card-body">
            <form @submit.prevent>
                <h5 class="mb-3 text-center">
                    هل انت متاكد من حذف هذا @{{ cat_name }} القسم?
                </h5>
                <h6 class="mb-3 text-center">
                    ملحوظة سيتم حذف مقالات القسم بالكامل
                </h6>
...
createApp({
    ...
    methods: {
        ...
        async getCategories() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/admin/categories/main?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
