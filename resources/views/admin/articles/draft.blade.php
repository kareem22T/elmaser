...
<div class="card w-100" id="word_prev">
    <div class="card-header d-flex justify-content-between gap-3">
        ...
        <div class="btns d-flex gap-2">
            ...
            <a href="/admin/" class="btn btn-success w-fit d-flex gap-2 align-items-center">
              المقالات المنشورة 
            </a>
...
createApp({
    ...
    methods: {
        ...
        async getarticles() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/admin/articles/draft?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/admin/articles/draft/search?page=${this.page}`, {
