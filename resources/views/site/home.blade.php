        ...
        <div style="max-width: 100vw;">
            <div class="container">
                                    ...
                                    @if ($main_article)
                                        <div class="swiper-slide">
                                            ...
                                            <div class="text" style="width: 100%">
                                                <div class="head">
                                                    <a href="/category/{{ $main_article->category?->id }}"
                                                        style="color: #fff; text-decoration: none">
                                                        <i class="fa-solid fa-list"></i>
                                                        {{ $main_article->category?->main_name }}
                                                    </a>
                                                </div>
                                                <a href="/article/{{ $main_article->id }}"
                    ...
                    @if (isset($settingsArray['square_ad']) && $settingsArray['square_ad']['value'])
                        <div class="side-ad">
                            @if (isset($settingsArray['square_ad_url']) && $settingsArray['square_ad_url']['value'])
                                <a href="{{ $settingsArray['square_ad_url']['value'] }}" target="_blank"
                ...
                <div class="digital_coins">
                    <div>
                        @if (isset($settingsArray['gold_24']) &&
                                $settingsArray['gold_24']['value'] &&
                                isset($settingsArray['gold_21']) &&
                                $settingsArray['gold_21']['value'] &&
                                isset($settingsArray['gold_18']) &&
                                $settingsArray['gold_18']['value']
                        )
                            <div class="prices">
                                <h2><ion-icon name="pricetags-outline"></ion-icon> اسعار اليوم</h2>
                                <div>
                                    <h4><img src="./assets/imgs/gold_icon.png" alt=""> الذهب عيار 18:
                                        <span>{{ $settingsArray['gold_24']['value'] }} ج</span></h4>
                                    <h4><img src="./assets/imgs/gold_icon.png" alt=""> الذهب عيار 21:
                                        <span>{{ $settingsArray['gold_21']['value'] }} ج</span></h4>
                                    <h4><img src="./assets/imgs/gold_icon.png" alt=""> الذهب عيار 24:
                                        <span>{{ $settingsArray['gold_18']['value'] }} ج</span></h4>
                                </div>
                            </div>
                        @endif
                        @if (isset($settingsArray['horizon_sec_ad']) && $settingsArray['horizon_sec_ad']['value'])
                            <div class="top_ad">
                                @if (isset($settingsArray['horizon_sec_ad_url']) && $settingsArray['horizon_sec_ad_url']['value'])
                                    <a href="{{ $settingsArray['horizon_sec_ad']['value'] }}" target="_blank"
                                        style="text-decoration: none">
                                @endif
                    ...
                    @if (isset($settingsArray['square_sm_ad']) && $settingsArray['square_sm_ad']['value'])
                        <div class="ad-left-2">
                            @if (isset($settingsArray['square_sm_ad_url']) && $settingsArray['square_sm_ad_url']['value'])
                                <a href="{{ $settingsArray['square_sm_ad_url']['value'] }}" target="_blank"
                                    style="text-decoration: none">
                            @endif
                                ...
                                @foreach ($latest_news as $item)
                                    <a href="/article/{{ $item->id }}" class="swiper-slide">
                                        ...
                                        <div class="text">
                                            <p>{{ $item->title }}</p>
                                            <span
                                                class="date">{{ $days[Carbon\Carbon::parse($item->created_at)->dayOfWeek] . ', ' . Carbon\Carbon::parse($item->created_at)->day . ' ' . $months[Carbon\Carbon::parse($item->created_at)->month - 1] . ', ' . Carbon\Carbon::parse($item->created_at)->year . ' ' . str_replace(['AM', 'PM'], ['ص', 'م'], Carbon\Carbon::parse($item->created_at)->format('h:i A')) }}</span>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                ...
                <div class="container">
                    @if ($categories_per_home && count($categories_per_home) > 0)
                        @foreach (array_slice($categories_per_home, 0, 4) as $index => $category)
                            <div>
                                <h4>{{ $category?->main_name }} <ion-icon name="ellipsis-horizontal-outline"></ion-icon>
                                </h4>
                ...
                @if (isset($settingsArray['horizon_third_ad']) && $settingsArray['horizon_third_ad']['value'])
                    <div class="top_ad">
                        @if (isset($settingsArray['horizon_third_ad_url']) && $settingsArray['horizon_third_ad_url']['value'])
                            <a href="{{ $settingsArray['horizon_third_ad']['value'] }}" target="_blank"
                                style="text-decoration: none">
                        @endif
                            ...
                            @foreach ($more_visited as $visit)
                                @if ($visit->article)
                                    <div>
                                        ...
                                        <div class="text">
                                            <a href="category/{{ $visit->article->category?->id }}">
                                                {{ $visit->article->category?->main_name }} <i
                                                    class="fa-solid fa-arrow-up-right-from-square"></i></a>
                                            <a href="/article/{{ $visit->article->id }}">{{ $visit->article->title }}</a>
                                            <span
                                                class="date">{{ $days[Carbon\Carbon::parse($visit->article->created_at)->dayOfWeek] . ', ' . Carbon\Carbon::parse($visit->article->created_at)->day . ' ' . $months[Carbon\Carbon::parse($visit->article->created_at)->month - 1] . ', ' . Carbon\Carbon::parse($visit->article->created_at)->year . ' ' . str_replace(['AM', 'PM'], ['ص', 'م'], Carbon\Carbon::parse($visit->article->created_at)->format('h:i A')) }}</span>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                    ...
                    <div class="borsa-2">
                        <div class="container">
                            <h1 class="head-dark head"><i
                                    class="fa-solid fa-chart-simple"></i>{{ $category?->main_name }} <span
                                    class="line"></span>
                            </h1>
                            <div class="swiper borsaSlider">
                                <div class="swiper-wrapper">
